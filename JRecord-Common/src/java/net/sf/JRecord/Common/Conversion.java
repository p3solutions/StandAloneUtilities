...
public final class Conversion {
	...
	// private static final DecimalFormatSymbols decSymbols = new
...
//	private static final NumberFormat numberFormat = NumberFormat.getNumberInstance(Locale.US);
	...
	public static String getString(final byte[] record, final int start, final int fin, final String fontName) {
		...
		if (fin - start <= 0) {
			...
			try {
				...
				// System.out.println("Conversion Error 1 " + e.getMessage() + " >> " +
				// fontName);
				s = new String(record, start, fin - start);
			}
		}
	 ...
	 */
	public final static String toString(final byte[] record, final String fontName) {
		String s = "";
...
//					+ " " + bytes[0]);
			throw new RecordException(VALUE_IS_TO_BIG_FOR_FIELD,
	...
	/**
	 * Load a long into the Record (Little Endian format -> low to high byte).
	 *
	 * @param record     Full record which is to be updated
	 ...
	 *
	 */
	public static void setLongLow2High(byte[] record, int pos, int len, long val, boolean isPositive) {
		int i;
...
//		}
	...
	public static final String numTrim(String s, char decimalCharacter) {
		...
		if (i > 0) {
			// System.out.println(" @@@ " + i + " " + s.charAt(i) + " " + decimalCharacter +
			// " !" + s);
	...
	public final static String decodeCsvField(String pDelim, String fontName, char defaultChar) {
		...
		if ((pDelim == null) || (pDelim = pDelim.trim()).equalsIgnoreCase("<tab>")
				|| pDelim.equalsIgnoreCase("<default>") || "\\t".equals(pDelim)) {
			delimiter = "\t";
	...
	/**
	 * The input to this method can be either:
	 * <ul>
	 * <li>A single character
	 * <li>A character represented in unicode format: \\u0001 (\\u followed by the
	 * character code in hex format).
	 * </ul>
	 * 
...
//			}
	...
	public static boolean isHtml(String s) {
		...
		return field2check.indexOf('<') >= 0 && (field2check.startsWith("<html>") || field2check.indexOf("<h1>") >= 0
				|| field2check.indexOf("<h0>") >= 0 || field2check.indexOf("<h2>") >= 0
				|| field2check.indexOf("<b>") >= 0 || field2check.indexOf("<i>") >= 0
				|| field2check.indexOf("<em>") >= 0 || field2check.indexOf("<li>") >= 0);
	}

	public static String padFront(String val, int size, char ch) {
		return new StringBuilder().append(getCharArray(size, ch)).append(val).toString();
	}
	...
	public static final void setDefaultSingleByteCharacterset(String defaultSingleByteCharacterset) {
	...
	public static final void setAlwaysUseDefaultSingByteCharset(boolean alwaysUseDefaultSingByteCharset) {
