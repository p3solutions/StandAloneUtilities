...
/**
 * This class is the base for all numeric-Type classes + it handles all
 * character based numeric types (apart from zoned)
 *
 * @author Bruce Martin
 *
 * @version 0.55
...
//	private static final int BASE_10 = 10;
	 ...
	 *
	 * <p>
	 * <b>Note:</b> A "Type" is the interface between the raw data in the file and
	 * what is to be displayed on the screen. A "Type" class needs to perform all
	 * conversions file and the screen.
	 *
	 ...
	 */
	protected TypeNum(final boolean leftJustified, final boolean adjustDecimal, final boolean couldBeALong,
			final boolean isPositive, final boolean binary, final boolean couldBeHexHero,
	 ...
	 */
	public Object getField(final byte[] record, final int position, final IFieldDetail currField) {
		String s = "";
		...
		if (!Pattern.matches(regex, s.trim())) {
			String str = currField.getName() + " " + s + " Value Changed to Null with regex" + "\n";
			try {
...
//	}
	...
	protected String addDecimalPoint(String s, int decimal) {
		...
		if (((decimal != 0) && adjustTheDecimal && (!s.endsWith(" ")) && Conversion.isInt(s))) {
			if (s.startsWith("-")) {
				s = s.substring(1);
				sign = "-";
			} else {
			...
			if (decimal > 0) {
				...
				s = sign + Conversion.numTrim(s.substring(0, len - decimal)) + Conversion.getDecimalchar()
	...
	protected final byte[] setFieldToVal(byte[] record, final int position, final IFieldDetail field, String val) {
...
//			        java.lang.Math.pow(BASE_10, field.getDecimal())));
	...
	private void checkCharNumLength(String val, int length) throws RecordException {
		...
		if (val.length() > length) {
			throw new RecordException("Value {0} is to big !! {1} > {2}", new Object[] { val, val.length(), length });
		}
	}

	/**
	 * get the positive value. This attribute was originally setup for
	 * <b>internal</b> use (with binary numbers). I will try and fix it for other
	 * types, but it might / might not be set correctly. Use it at you own risk !!
	 * 
