		...
		for (byte v3 = 0; v3 < 10; v3++) {
			...
			for (int v2 = 0; v2 < 10; v2++) {
				for (int v1 = 10; v1 < 16; v1++) {
					...
					if (v1 != 13) {
						assertEquals("id: " + v1, new StringBuilder("000").append(v3).append(hexDigits[v2]).toString(),
								Conversion.getMainframePackedDecimal(bytes, 0, 3, null));
						assertEquals(new StringBuilder("0").append(v3).append(hexDigits[v2]).toString(),
								Conversion.getMainframePackedDecimal(bytes, 1, 2, null));
					} else {
