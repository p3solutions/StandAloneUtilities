...
public class StartProcess {
	...
	private void processFile(File file) throws IOException {
		...
		while ((line = br.readLine()) != null) {
			...
			count++;
			if (count == 1) {
				fileName = line.substring(line.indexOf(ipargs.getDelimeter()) + 1,
						line.lastIndexOf(ipargs.getDelimeter()));
				date = line.substring(line.lastIndexOf(ipargs.getDelimeter()) + 1, line.length());
		...
		int destCount = Integer
				.valueOf(lastline.substring(lastline.indexOf(ipargs.getDelimeter()) + 1, lastline.length()));
	...
	protected void createReport(StringBuffer sb, String path) {
		try {
			...
			String finalfile = path + File.separator + "Count Validation Report_" + reportId + ".pdf";
